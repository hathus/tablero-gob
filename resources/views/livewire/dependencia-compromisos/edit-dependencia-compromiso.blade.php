<x-slot name="header">
    <div class="uppercase flex items-center space-x-4">
        <h2 class="text-sm font-semibold text-black dark:text-gray-200 leading-tight">
            {{ __('Editar Avance de Compromiso') }}
        </h2>
    </div>
    ...
    <div class="mt-4">
        <a href="{{ route('avance-compromisos') }}"
            class="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-rosains dark:bg-gray-700 text-white dark:text-gray-200 font-semibold hover:bg-gray-300 dark:hover:bg-gray-600 transition duration-300 shadow-sm text-sm"
...
<form
    ...
    wire:submit.prevent="edit" novalidate x-data="{ loading: false }" {{-- showInputs y showForm eliminados --}}
    ...
    @if (session()->has('message'))
        <div x-data="{ show: true }" x-init="setTimeout(() => show = false, 4000)" x-show="show" x-transition
            class="text-sky-700 dark:text-sky-400 bg-sky-100 border border-doradoins rounded-lg px-4 py-3 text-center font-semibold">
            {{ session('message') }}
        </div>
    @endif
        ...
        <div class="mt-4">
            <div class="relative" wire:ignore>
                ...
                <select wire:model="tipo_compromiso_id" wire:model.live="tipo_compromiso_id" name="tipo_compromiso_id"
                    style="width: 100%;" id="tipo_compromiso_id"
    ...
    <div id="fase4" class="border p-3 rounded-lg border-doradoins">
        {{-- Meta Alcanzada --}}
        <div
            class="bg-white dark:bg-teal-600 overflow-hidden shadow-sm sm:rounded-lg p-2 text-rosains mt-3 border-b-pink-500 border-b-2">
            Meta Alcanzada <br />
            Ingrese la cantidad de acciones realizadas entre 2021 - 2025.
        ...
        <div class="mt-4">
            ...
            <div class="mt-4">
                ...
                <div class="mt-4">
                    <div class="relative" wire:ignore>
                        <label for="fuente_informacion"
                            class="block mb-1 text-sm font-medium text-gray-600 dark:text-gray-300 uppercase">
                            Fuente de Información
                        </label>
                        <select wire:model="fuente_informacion" wire.model.live="fuente_informacion"
                ...
                <div class="mt-4">
                    <x-input-label class="uppercase" for="medio_verificacion" :value="__('Medios de Verificación')" />
    ...
    <x-secondary-button class="w-full justify-center mt-4" wire:click='cancel()'>
        Cancelar Actualización de Avance
    </x-secondary-button>
</form>
...
@script
    <script>
        $('.js-compromisos').select2({ // Esta es la clase del componente
            ...
            width: 'resolve',
            placeholder: 'Seleccione un Compromiso',
        }).on('change', function() {
            let data = $(this).val(); //Estos son los datos que se envían a la base de datos
            $wire.set('compromiso_id', data);
        }).val('{{ $compromiso_id }}').trigger('change'); // Esto es para que se muestre el valor seleccionado
        ...
        }).val('{{ $tipo_recurso_id }}').trigger('change'); // Esto es para que se muestre el valor seleccionado
        ...
        }).val('{{ $tipo_compromiso_id }}').trigger('change'); // Esto es para que se muestre el valor seleccionado
        ...
        }).val('{{ $tipo_cobertura }}').trigger('change'); // Esto es para que se muestre el valor seleccionado
        ...
        }).val('{{ $unidad_medida_sel_id }}').trigger('change');
        ...
        $('.js-fuente-informacion').select2({
            theme: 'tailwindcss-3',
            multiple: true,
            placeholder: 'Seleccione una o mas Fuente(s) de Información',
        }).on('change', function() {
            let data = $(this).val();
